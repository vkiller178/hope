...
class Stackedit {
  ...
  openFile(_file = {}, silent = false) {
    // Close before opening a new iframe
    this.close()

    const file = { ...this.$file, ..._file }
    this.$file = file
    this.$wrapper = file.wrapper || document.querySelector('body')

    // Make StackEdit URL
    this._urlParser.href = this.$options.url
    this.$origin = `${this._urlParser.protocol}//${this._urlParser.host}` // Save StackEdit origin
    ...
    const serializedParams = Object.keys(params)
      ...
      .join('&')
    this._urlParser.hash = `#${serializedParams}`
  ...
  close() {
    if (this.$messageHandler) {
      ...
      this.$wrapper.removeChild(this.$containerEl)
